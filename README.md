...
`start_date`/`end_date` (неділя 00:00 → наступна неділя 00:00 за
...
Щонеділі о 00:00 (`php artisan week:close`, через Laravel Scheduler)
...
`floor(дистанція_км / 100)` — 1 Torcoin за кожні повні 100 км. Рахується
окремо за весь час і за поточний рік.
